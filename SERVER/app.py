...
cred = credentials.Certificate("reactfyp-9d8ca-firebase-adminsdk-4v4dd-c0279fc2d0.json")
...
def find_meal_combinations(total_calories, allergies, cuisine, diet_type):
    """Find meal combinations that sum up to the total calorie value."""
    ...
    # Find all combinations of breakfast, lunch, and dinner items
    valid_combinations = []
...
def recommend():
    if request.method == "POST":
        ...
        if len(total_calories) == 0:
            ...
            total_calories = re.findall(r'Total Calories:(.*?)$', results, re.DOTALL)
        if len(total_calories) == 0:
            dinner_names = re.findall(r'Dinner:(.*?)Total:', results, re.DOTALL)
            total_calories = re.findall(r'Total:(.*?)$', results, re.DOTALL)
        if len(total_calories) == 0:
            dinner_names = re.findall(r'Dinner:(.*?)Total Calories for the Day:', results, re.DOTALL)
            total_calories = re.findall(r'Total Calories for the Day:(.*?)$', results, re.DOTALL)
        
        # try:
        #     total_calories = total_calories[0].split('calories')
        # except IndexError:
        #     total_calories = data.get('total_calories')
...
def recommend_meals():
    ...
    meal_combinations = find_meal_combinations(total_calories, allergies, cuisine, diet_type)
    
    if not meal_combinations:
        return jsonify({"message": "No valid combinations found for the given criteria."})
    
    return jsonify({"Meal Combinations": meal_combinations})
...
def recommend_meals2():
    ...
    # Prepare response in the required format and remove duplicates
